...
const metrics = [
  { key: 'used', value: filesize(totalFileSizeInBytes.value) },
  { key: 'remaining', value: filesize(remainingStorageSize.value) },
  { key: 'usedPercent', value: totalFileSizeInPercentage },
  { key: 'free', value: remainingStorageSizeInPercentage }
]
  ...
  >
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        <span class="text-sm text-zinc-500 dark:text-zinc-400">
          <span>{{ t('domain.plan.storage.title') }}</span>
        </span>
    ...
    <div class="grid grid-cols-2 gap-2 text-xs md:grid-cols-4">
      <div class="flex flex-col" v-for="metric in metrics">
        <span class="text-zinc-400">{{ t(`domain.plan.metrics.${metric.key}`) }}</span>
