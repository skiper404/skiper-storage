...
const { activeAction, selectedReason, close, setAction, title, description, isConfirmDisabled } = useAdminUserActions()

const handleClick = async () => {
  if (activeAction.value === 'unblock') {
    unblockUser(props.user.id)
    toast.success(`Пользователь ${props.user.username} разблокирован.`)
  }

  if (activeAction.value === 'block') {
    blockUser(props.user.id, selectedReason.value)
    toast.success(`Пользователь ${props.user.username} заблокирован.`)
  }

  if (activeAction.value === 'delete') {
    deleteUser(props.user.id, selectedReason.value)
    toast.success(`Пользователь ${props.user.username} и все его файлы безвозвратно удалены.`)
  }
  close()
  await executeUsers()
}
